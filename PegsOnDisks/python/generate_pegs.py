...
def main():
  ...
  # objects
  objectHeight = [0.03, 0.10]
  objectRadius = [0.01, 0.02]
  nObjects = 1000
  ...
  for objIdx in xrange(nObjects):
    ...
    obj = rlEnv.GenerateCylinderMesh(objectHeight, objectRadius, "peg-{}".format(objIdx))
    ...
    savemat("peg-{}.mat".format(objIdx), data)

    rlEnv.PlotCloud(cloud)
    if plotImages: point_cloud.Plot(cloud)
    if showSteps: raw_input("Placed peg-{}.".format(objIdx))
