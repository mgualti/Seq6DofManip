#!/usr/bin/env python
'''Trains an HSA agent to perform the upright pegs on disks task.'''
...
def Main():
  ...
  for episode in xrange(nEpisodes):
    ...
    R = 0; holdingDesc = None; observations = []; actions = []; rewards = []
    for t in xrange(tMax):
      #  get the next action
      o, a, overtDesc, epsilon = rlAgent.SenseAndAct(holdingDesc, t, rlEnv, episode >= unbiasOnEpisode)
      # perform transition
